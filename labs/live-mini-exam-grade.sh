#!/bin/bash
# first draft, pre-test, 14 Aug. 2025
# 
...
then
	...
	if [[ $(ls -ld /data/sales | awk '{ print $3 }') == linda ]]
        then
                echo -e "\033[32m[OK]\033[0m\t\t \033[1m/data/sales\033[0m is owned by user linda"
                SCORE=$(( SCORE + 10 ))
        else
                echo -e "\033[31m[FAIL]\033[0m\t\t \033[1m/data/sales\033[0m is not user-owned by user linda"| tee -a /tmp/exam-report.txt
        fi
	if [ -g /data/sales ]
	then
	...
	if [ -u /data/sales ]
	then
echo -e "\033[32m[OK]\033[0m\t\t \033[1m/data/sales\033[0m has the SUID permission"
                SCORE=$(( SCORE + 10 ))
        else
                echo -e "\033[31m[FAIL]\033[0m\t\t \033[1m/data/sales\033[0m doesn't have the SUID permission"| tee -a /tmp/exam-report.txt
        fi
else
...
###task 8
echo -e "\033[1mevaluating task 8\033[0m"| tee -a /tmp/exam-report.txt
if grep passwd /root/sugidfiles.txt &>/dev/null
then
        echo -e "\033[32m[OK]\033[0m\t\t the /root/sugidfiles.txt contains SUID files"
        SCORE=$(( SCORE + 10 ))
else
        echo -e "\033[31m[FAIL]\033[0m\t\t the /root/sugidfile.txt failed the test on SUID files"| tee -a /tmp/exam-report.txt
fi
TOTAL=$(( TOTAL + 10 ))

###task 9
echo -e "\033[1mevaluating task 9\033[0m"| tee -a /tmp/exam-report.txt
echo -e "\033[37mI need to optimize grading this task\033[0m"
if crontab -u root -l | grep sugidcheck &>/dev/null
then
        echo -e "\033[32m[OK]\033[0m\t\t the sugidcheck script is scheduled to run as a user root cron job"
        SCORE=$(( SCORE + 10 ))
else
        echo -e "\033[31m[FAIL]\033[0m\t\t the sugidcheck script is not scheduled as a user root cron job"| tee -a /tmp/exam-report.txt
fi
TOTAL=$(( TOTAL + 10 ))
if grep diff /root/sugidcheck &>/dev/null
then
        echo -e "\033[32m[OK]\033[0m\t\t the sugidcheck script uses diff to check file differences"
        SCORE=$(( SCORE + 10 ))
else
        echo -e "\033[31m[FAIL]\033[0m\t\t the sugidcheck script does not use diff to check file differences"| tee -a /tmp/exam-report.txt
fi
TOTAL=$(( TOTAL + 10 ))
